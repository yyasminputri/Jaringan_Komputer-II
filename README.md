...
- Explanation
1. Mengedit File Virtual Host Apache dengan menyesuaikan konfigurasi virtual host untuk subdomain www.k1.vitamin.brokoli.A03.com, lakukan ` nano /etc/apache2/sites-available/k1.vitamin.brokoli.A03.com.conf` untuknya. 
```
<VirtualHost *:9696>
...
**TESTING**
```
http://www.k1.vitamin.brokoli.A03.com:9696
http://www.k1.vitamin.brokoli.A03.com:8888
```
...
- Explanation
  ...
  Memakai htpasswd untuk menyimpan kredensial

  
  ```
  htpasswd -c /etc/apache2/.htpasswd Sebla
   <Directory /var/www/k1.vitamin.brokoli.A03.com>
        Options Indexes FollowSymLinks
        AllowOverride All
...
- Explanation
  ...
  tambahkan saja rule untuk memaksa redirect ke domain


  `Redirect "/" "http://www.brokoli.A03.com/"
...
- Explanation
  ...
  Harusnya cuma bikin redirect aja sama rewrite rule tapi masih error
```
  RewriteEngine On
  RewriteCond %{REQUEST_FILENAME} !-d
  RewriteCond %{REQUEST_URI} !^/public/images/vitamin.png$
  RewriteRule ^(.*)\.(jpg|jpeg|png|gif)$ /public/images/vitamin.png [L]
```
